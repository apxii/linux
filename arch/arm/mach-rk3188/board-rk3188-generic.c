 ...
 * Copyright (C) 2013-2014 Omegamoon Ltd.
 *
...
// Omegamoon >> Disable to use RBOX driver, else TABLET driver is used
#undef USE_TABLET_VIDEO_DRIVER  

//#ifdef USE_TABLET_VIDEO_DRIVER
...
#endif  // CONFIG_BACKLIGHT_RK29_BL
...
/* >>> Omegamoon - Added additional UMP buffer
	[1] = {
	.name  = "ump1 buf",
	.start = 0,
	.end   = 0,
...
static int rk_hdmi_enter_suspend(void)
{
	...
	printk("Omegamoon >> HDMI entering suspend\n");
	gpio_set_value(RK_HDMI_RST_PIN, GPIO_LOW);
...
{
	struct regulator *ldo_hdmi_12;
	printk("Omegamoon >> HDMI exit suspend\n");
...
static struct rkdisplay_platform_data tv_data = {
	...
	// Omegamoon: .io_switch_pin is used in rk1000_codec as speaker pin
	.io_switch_pin	= CONFIG_RK1000_SWITCH_GPIO,
};
#endif
...
/*		
		// Omegamoon >> Added support for 2 UMP buffers
		resource_mali[0].start = board_mem_reserve_add("ump0 buf", ump_mem_phy_size); 
		resource_mali[0].end = resource_mali[0].start + ump_mem_phy_size -1;

		resource_mali[1].start = board_mem_reserve_add("ump1 buf", ump_mem_phy_size); 
		resource_mali[1].end = resource_mali[1].start + ump_mem_phy_size -1;
*/
	}
#endif
#endif
