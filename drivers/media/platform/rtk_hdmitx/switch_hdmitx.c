...
static struct extcon_dev *edev = NULL;
...
static ssize_t hdmitx_switch_print_state(struct extcon_dev *edev, char *buffer)
{
...
static int hdmitx_switch_thread(void *arg)
{
	...
	hdmitx_device_t *pdev = container_of(edev, hdmitx_device_t, edev);
	...
	for (;;) {
		...
		if (state != s_data.state) {
			...
			s_data.state = state;
			extcon_set_state(edev, s_data.state);
...
static void hdmitx_switch_work_func(struct work_struct *work)
{
	...
	if (state != extcon_get_state(edev)) {
		s_data.state = state;
		extcon_set_state(edev, s_data.state);
...
int register_hdmitx_extcondev(hdmitx_device_t *device)
{
	int ret;
	int hpd_state;
	...
	edev->name = HDMI_SWITCH_NAME;
	edev->print_state = hdmitx_switch_print_state;
	...
	hdmitx_hpd_tsk = kthread_run(hdmitx_switch_thread, edev, "hdmitx_hpd_thread");
...
void deregister_hdmitx_extcondev(hdmitx_device_t *device)
{
	return extcon_dev_unregister(&device->edev);
}
EXPORT_SYMBOL(deregister_hdmitx_extcondev);
...
int rtk_hdmitx_switch_suspend(void)
{
	...
	pdev = container_of(edev, hdmitx_device_t, edev);
