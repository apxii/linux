...
int sunxi_tcon_common_check_line(void *data)
{
    ...
    cur_line = tcon_get_cur_line(sunxi_enc->enc_id, sunxi_enc->enc_id);
    start_delay = tcon_get_start_delay(sunxi_enc->enc_id, sunxi_enc->enc_id);
...
int sunxi_tcon_hdmi_set_timing(void *data, struct drm_display_mode *mode)
{
    ...
    DRM_DEBUG_KMS("[%d] encoder_id: %d vic:%d\n",__LINE__,
        ...
        int fix_crtc, int id, struct sunxi_hardware_res *hw_res)
{
	...
	DRM_INFO("[%d] encoder_id:%d(%d) possible_crtcs:0x%x\n", __LINE__,
